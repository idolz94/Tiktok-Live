...
export function normalizeApiOrderForUi(order: any): OrderWithTikTok {
  ...
  return {
    ...
    avatar: String(order?.avatar || order?.customerAvatarUrl || order?.customer_avatar_url || order?.avatarUrl || order?.avatar_url || ""),
    avatarUrl: String(order?.avatarUrl || order?.customerAvatarUrl || order?.customer_avatar_url || order?.avatar || order?.avatar_url || ""),
    comment,
