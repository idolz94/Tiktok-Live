    ...
    <div className="bg-white px-4 pt-2 pb-4">
      ...
      <div className="mt-4 grid grid-cols-2 gap-2">
        {summaries.map((item) => {
          ...
          return (
            <button key={item.key} type="button" onClick={() => onChangeFilter(isActive ? "all" : item.key)} className={`flex items-center gap-3 rounded-xl border border-black/10 p-4 text-left ${item.cardClassName} ${isActive ? "ring-2 ring-black/15" : ""}`}>
              <span className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full ${item.iconClassName}`}>{item.icon}</span>
              <span className="min-w-0"><strong className="block text-[18px] leading-6 font-semibold text-black">{item.value}</strong><span className="block text-[12px] leading-[18px] text-[#484848]">{item.label}</span></span>
            </button>
          );
        })}
      </div>
      ...
      <div className="mt-4 flex items-center justify-between">
        ...
        <button type="button" onClick={() => onChangeFilter(activeFilter === "all" ? "unpaid" : "all")} className="inline-flex h-9 items-center gap-1.5 rounded-full px-2 text-[14px] font-medium text-black">
          <FilterIcon />
          Filter
        </button>
      </div>
      ...
      {searchText ? (
        <div className="mt-3 flex items-center gap-2 rounded-full bg-[#f2f2f2] px-3 py-2">
          <input value={searchText} onChange={(event) => onChangeSearch(event.target.value)} placeholder="Tìm kiếm" className="min-w-0 flex-1 bg-transparent text-sm text-black outline-none" />
