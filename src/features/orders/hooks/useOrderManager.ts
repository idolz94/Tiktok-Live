...
export function useOrderManager({
  ...
  const [shipmentOrders, setShipmentOrders] = useState<OrderWithTikTok[]>([]);
  ...
  const reloadShipmentOrders = useCallback(async () => {
    try {
  ...
  const updateProductInOrder = useCallback((orderId: string, itemId: string, updates: Partial<OrderProduct>) => {
    setOrders((prev) =>
      prev.map((order) => {
        if (order.id !== orderId) return order;
        return {
          ...order,
          products: order.products.map((p) => (p.id === itemId ? { ...p, ...updates } : p)),
        };
      }),
    );
  }, []);
  ...
  return {
    ...
    getShippingTracking: getShippingTrackingApi,
    liveTab,
    setLiveTab,
    orderFilter,
    setOrderFilter,
